 ...
 * назад да аб'екта.*/
void AbstractGrQtItem::calculateObjectPosition(const QPointF &event_pos, const QPointF &prev_pos)
{
...
void AbstractGrQtItem::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* m_event)
{
    if(_communicator_)
    {
...
void AbstractGrQtItem::mouseReleaseEvent(QGraphicsSceneMouseEvent * m_event)
{
    if(flags()&ItemIsMovable)
    {
...
void AbstractGrQtItem::mouseMoveEvent(QGraphicsSceneMouseEvent* m_event)
{
    if(!(flags()&ItemIsMovable))
    {
        m_event->ignore();
...
void AbstractGrQtItem::hoverEnterEvent(QGraphicsSceneHoverEvent * h_event)
{
...
void AbstractGrQtItem::hoverLeaveEvent(QGraphicsSceneHoverEvent * h_event)
{
    _last_screen_pos_ = QPoint();
    if(_communicator_)
    {
...
void AbstractGrQtItem::setItemCommunicator(ItemCommunicator* communicator)
{
    if(!communicator)
    {
        return;
...
void AbstractGrQtItem::setGrY(coord_real y)
{
//    QPointF sc_pos(mapToScene(pos()));
//    sc_pos.setY(y);
//    setPos(mapFromScene(sc_pos));
...
coord_real AbstractGrQtItem::getGrX() const
{
    //QPointF sc_pos(mapToScene(pos()));
    return x();//sc_pos.x();
}

coord_real AbstractGrQtItem::getGrY() const
{
    //QPointF sc_pos(mapToScene(pos()));
    return y();//sc_pos.y();
}

void AbstractGrQtItem::setGrWidth(coord_real width)
{
    setRadius(static_cast<int>(width/2));
    return;
}

void AbstractGrQtItem::setGrHeight(coord_real height)
{
    setRadius(static_cast<int>(height/2));
    return;
}

coord_real AbstractGrQtItem::getGrWidth() const
{
    return boundingRect().width();
}

coord_real AbstractGrQtItem::getGrHeight() const
{
    return boundingRect().height();
}

void AbstractGrQtItem::moveGr(coord_real x, coord_real y)
{
    setPos(x,y);
    return;
}
...
void AbstractGrQtItem::collectClosestItems(QList<QGraphicsItem*>& item_container)
{
    ...
    {
        AbstractGrQtItem * gr_item = qgraphicsitem_cast<AbstractGrQtItem*>(item);
        if(!gr_item)
        {
            continue;
        }
...
void AbstractGrQtItem::calculateRepulsion(qreal& velocity_x,
                                        qreal& velocity_y,
...
void AbstractGrQtItem::calculateAttraction(qreal& velocity_x, qreal &velocity_y,
...
void AbstractGrQtItem::calcForce()
{
...
bool AbstractGrQtItem::advPosition()
{
    if(_adv_pos_ == pos())
    {
        return false;
